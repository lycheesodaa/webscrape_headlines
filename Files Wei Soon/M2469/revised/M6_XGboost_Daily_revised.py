...
result_folder = Path("/home/lianlian/webscrape_headlines/results/compare_models_daily/XGB")
...
# TopFeaturePercs = [0.5] # Select top % of the full set of features
GPU_ID = 0
train_size = 0.6
val_size = 0.2
test_size = 0.2
...
for train_time in range(0, train_times):
    ...
    for TopFeaturePerc in TopFeaturePercs:
        ...
        for output_step in output_steps:
            ...
            # Train the model for each step ahead using only training data
            y_train_flat = y_train.reshape(-1)
            X_train_expanded = np.repeat(X_train, output_step, axis=0)
            step_indicators_train = np.tile(np.arange(output_step), len(X_train)).reshape(-1, 1)
            X_train_expanded = np.hstack([X_train_expanded, step_indicators_train])
            
            # Create validation data for early stopping
            y_val_flat = y_val.reshape(-1)
            X_val_expanded = np.repeat(X_val, output_step, axis=0)
            step_indicators_val = np.tile(np.arange(output_step), len(X_val)).reshape(-1, 1)
            X_val_expanded = np.hstack([X_val_expanded, step_indicators_val])

            # Train the model with validation data for early stopping
            dtrain = xgb.DMatrix(X_train_expanded, y_train_flat)
            dval = xgb.DMatrix(X_val_expanded, y_val_flat)
            
            xgb_model.fit(
                X_train_expanded, 
                y_train_flat,
                eval_set=[(X_train_expanded, y_train_flat), (X_val_expanded, y_val_flat)],
                verbose=True,
            )

            # Make Predictions
            X_test_expanded = np.repeat(X_test, output_step, axis=0)
            step_indicators_test = np.tile(np.arange(output_step), len(X_test)).reshape(-1, 1)
            X_test_expanded = np.hstack([X_test_expanded, step_indicators_test])

            dtest = xgb.DMatrix(X_test_expanded)
            y_pred_flat = xgb_model.predict(X_test_expanded)
            y_pred = y_pred_flat.reshape(len(X_test), output_step)
            ...
            plt.title("Carbon Credit Price Multi-Step Forecasting with XGB")
            plt.xlabel("Time")
            ...
            # Save Plot
            plot_file = os.path.join(result_folder, f"XGB_MultiStep_feat{TopFeaturePerc}_lb{lookback}_pl{output_step}.png")
            ...
            results_file = os.path.join(result_folder, f"XGB_MultiStep_feat{TopFeaturePerc}_lb{lookback}_pl{output_step}.xlsx")
