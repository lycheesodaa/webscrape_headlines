...
result_folder = Path("/home/lianlian/webscrape_headlines/results/compare_models_daily/RF")
...
for train_time in range(0, train_times):
    ...
    for TopFeaturePerc in TopFeaturePercs:
        ...
        for output_step in output_steps:
            ...
            # Train Random Forest Model - ignore the warning, we are doing multi-output regression
            rf_model = RandomForestRegressor(n_estimators=200, random_state=42, verbose=1)
            rf_model.fit(X_train, y_train)  # Use only training data for model fitting
            ...
            # inverse scaling
            if instance_norm:
                y_pred_inverse = np.zeros_like(y_pred_normalized)
                y_test_inverse = np.zeros_like(y_test)
                for i, pred in enumerate(y_pred_normalized):
            ...
            plt.title("Carbon Credit Price Multi-Step Forecasting with RF")
            plt.xlabel("Time")
            ...
            # Save Plot
            plot_file = os.path.join(result_folder, f"RF_MultiStep_feat{TopFeaturePerc}_lb{lookback}_pl{output_step}.png")
            ...
            results_file = os.path.join(result_folder, f"RF_MultiStep_feat{TopFeaturePerc}_lb{lookback}_pl{output_step}.xlsx")
