...
result_folder = "/home/lianlian/webscrape_headlines/results/compare_models_monthly/RF"
...
def create_features_targets(X, y, lookback, forecast_horizon):
    if not instance_norm:
        ...
        for i in range(lookback, len(X) - forecast_horizon + 1):
            X_seq.append(X[i - lookback:i].flatten())
        ...
        for i in range(lookback, len(X) - forecast_horizon + 1):
            ...
            X_seq.append(X_normalized.flatten())
...
for forecast_horizon in forecast_horizons:
    ...
    for i_lookback, lookback in enumerate(lookbacks):
        ...
        # Train Random Forest Model on training data only
        ...
        # Save Plot
        plot_file = os.path.join(result_folder, f"RF_MultiStep_lb{lookback}_pl{forecast_horizon}.png")
        ...
        results_file = os.path.join(result_folder, f"RF_MultiStep_lb{lookback}_pl{forecast_horizon}.xlsx")
